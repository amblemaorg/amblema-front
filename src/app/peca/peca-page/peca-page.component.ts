...
export class PecaPageComponent {
  ...
  public generatePDF() {
    ...
    const generateThisPdf = async () => {
      ...
      // pdf images library
      if (this.pdfData["sponsorLogo"]) pdf.images({
      ...
      if (this.pdfData["sponsorLogo"]) pdf.add(new Canvas([new Polyline([{ x: 0, y: 0 },{ x: 0, y: 90 },{ x: 41, y: 115 },{ x: 45, y: 116 },{ x: 49, y: 115 },{ x: 90, y: 90 },{ x: 90, y: 0 }])
      ...
      if (this.pdfData["sponsorLogo"]) pdf.add(await new Img('sponsorLogo', true).fit([72,72]).absolutePosition(pdfPageSizes.width-126,15).build());
