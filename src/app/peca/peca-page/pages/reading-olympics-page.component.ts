...
import { readingOlympicsConfigMapper } from "./reading-olympics-config";
...
@Component({
  selector: "peca-reading-olympics",
  templateUrl: "../peca-page.component.html",
})
export class ReadingOlympicsPageComponent
    ...
    this.infoDataSubscription = this.infoData$
      .pipe(
        distinctUntilChanged(
          (prev, curr) =>
            JSON.stringify(
              prev.activePecaContent[`lapse${this.UrlLapse}`].readingOlympics
            ) ===
            JSON.stringify(
              curr.activePecaContent[`lapse${this.UrlLapse}`].readingOlympics
            )
        ),
        scan(
          (prev, curr) => {
          ...
          if (!this.isInstantiating) {
            if (data.activePecaContent) {
              if (!isNullOrUndefined(data)) {
                ...
                const config = readingOlympicsConfigMapper(
