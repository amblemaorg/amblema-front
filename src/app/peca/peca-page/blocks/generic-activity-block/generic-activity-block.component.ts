...
export class GenericActivityBlockComponent
  implements StructuralBlockComponent, OnInit, AfterViewInit
{
  @ViewChildren("genericActivityContainer", { read: ViewContainerRef })
  genericActivityContainer: QueryList<ViewContainerRef>;
  ...
  public instantiateChildBlocks() {
    ...
    this.settings.items.map((item, i) => {
      ...
      item.childBlocks.map((block, j) => {
        ...
        blockInstances.set(
          block.name || `genericactivity${i}block${j}`,
