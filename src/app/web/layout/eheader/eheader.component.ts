...
export class EheaderComponent implements OnInit {
  ...
  @Select(UserState.coordinator_modules_total)
  approved_modules_total$: Observable<number>;
  @Select(UserState.coordinator_modules) approved_modules$: Observable<
    CoordinatorModule[]
  >;
  ...
  setUserValues(user) {
    ...
    this.store
      .dispatch(new UpdateStepsProgress(user.project))
      .subscribe((res) => {
