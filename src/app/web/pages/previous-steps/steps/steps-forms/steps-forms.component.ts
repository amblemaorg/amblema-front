...
import {
  StateInfo,
  MunicipalityInfo,
} from "../../../../../models/steps/previous-steps.model";
...
export class StepsFormsComponent implements OnInit, OnDestroy {
  ...
  sponsorForm = this.fb.group({
    ...
    email: [
      "",
      [Validators.required, Validators.email, Validators.pattern(EMAIL_PTTRN)],
    ],
    rif: [
      "",
      [
    ...
    contactFirstName: [
      "",
      [Validators.required, Validators.pattern(LETTERS_PTTRN)],
    ],
    contactLastName: [
      "",
      [Validators.required, Validators.pattern(LETTERS_PTTRN)],
  ...
  schoolForm = this.fb.group({
    ...
    code: [
      "",
      [Validators.required, Validators.pattern(LETTERS_NUMBERS_PTTRN)],
    ], //str(codigo de la escuela),
    ...
    principalEmail: [
      "",
      [Validators.required, Validators.pattern(EMAIL_PTTRN)],
    ], //str,
    principalPhone: [
      "",
      [Validators.required, Validators.pattern(NUMBER_PTTRN)],
    ], //str solo numero,
    //
        ...
        this.sponsorForm.get("companyType").statusChanges.subscribe((res) => {
          if (
            ...
            this.sponsorForm.setControl(
              "companyOtherType",
              this.fb.control(this.sponsorForm.get("companyOtherType").value, [
                Validators.required,
          ...
          .statusChanges.subscribe((res) => {
            if (
              ...
              const addressData = this.municipalitiesData.filter((s) => {
                return (
                  s.id === this.schoolForm.get("addressMunicipality").value
                );
              });
      ...
      if (this.globals.isBrowser) {
        const initMap = () => {
          if (google) {
            ...
            this.showMap = true;
            setTimeout(() => {
              if (this.schoolmap) this.mapInitializer(this.disable);
            });
          }
        };
        let interval = null;
  ...
  private postForm(solicitudBody, fo, type) {
    this.stepsService.requestsFind(type, solicitudBody).subscribe(
      (res) => {
  ...
  private fillCoordinator(res) {
    this.coordinatorForm.setValue({
      ...
      birthdate: this.globals.getDateFormat(
        res.birthdate ? new Date(res.birthdate) : new Date()
      ),
  ...
  controlDate() {
    return (
      this.coordinatorForm.controls["birthdate"].value &&
  ...
  isNotValidDate(date) {
    if (date instanceof Date) {
      ...
      const isValid = this.globals.validateDate(
        dateString,
        "lower",
        true,
        true,
  ...
  hasMaxOrMin(controlName: string, controlCardType: string) {
    return (
      this.isMaxLenOver(controlName, controlCardType) ||
